  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-900 via-teal-900 to-purple-900">
      <div className="max-w-lg w-full bg-gradient-to-br from-green-800/80 via-teal-800/80 to-purple-800/80 rounded-2xl p-8 shadow-2xl animate-fade-in">
        <h1 className="text-2xl font-bold text-white mb-4">{t('results.title')}</h1>
        {matches.length > 0 && (
          <div className="mb-8">
            <h2 className="text-xl font-bold text-green-200 mb-2">{t('results.subtitle')}</h2>
            <ul className="space-y-4">
              {matches.map((match, idx) => (
                <li key={idx} className="bg-white/10 rounded-lg p-4">
                  ...
                  <div className="text-green-100 text-sm mb-1">{t('results.reasons')}: {Array.isArray(match.fitReasons) && match.fitReasons.join(', ')}</div>
                  <div className="text-green-100 text-sm mb-1">{t('results.strongSkills')}: {Array.isArray(match.strongSkills) && match.strongSkills.join(', ')}</div>
                  <div className="text-green-100 text-sm mb-1">{t('results.skillsToImprove')}: {Array.isArray(match.skillsToImprove) && match.skillsToImprove.join(', ')}</div>
                  {match.salaryRange && (
                    <div className="text-green-100 text-sm mb-1">{t('results.salary')}: Junior {match.salaryRange?.junior}, Mid {match.salaryRange?.mid}, Senior {match.salaryRange?.senior}</div>
                  )}
                  {match.education && (
                    <div className="text-green-100 text-sm mb-1">{t('results.education')}: {t('results.offline')}: {Array.isArray(match.education?.offline) && match.education.offline.join(', ')}, {t('results.online')}: {Array.isArray(match.education?.online) && match.education.online.join(', ')}</div>
                  )}
                  <div className="text-green-100 text-sm">{t('results.nextSteps')}: {Array.isArray(match.nextSteps) && match.nextSteps.join(', ')}</div>
                </li>
              ))}
            </ul>
          </div>
