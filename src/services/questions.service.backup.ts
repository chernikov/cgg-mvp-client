...
class QuestionsService {
  ...
  async migrateQuestionsFromCode(): Promise<void> {
    try {
      ...
      // Якщо все ж потрібно виконати міграцію, розкоментуйте код нижче
      /*
      console.log("🔄 Починаємо міграцію питань...");
      
      // Перевіряємо підключення до Firebase
      console.log("🔥 Перевіряємо підключення до Firebase...");
      
      // Імпортуємо всі питання з файлу
      console.log("📁 Імпортуємо питання з файлу...");
      const { 
        questions, 
      ...
      console.log("📊 Знайдено питання:", {
        basicQuestions: questions?.length || 0,
        magicalQuests: Object.keys(magicalQuestQuestions || {}).length,
        teacherQuestions: teacherSurveyQuestions?.length || 0,
        parentQuestions: parentSurveyQuestions?.length || 0
      });

      // Міграція... (код закоментований)
      */
      ...
      console.log("� Знайдено питання:", {
        ...
        teacherQuestions: teacherSurveyQuestions?.length || 0,
        parentQuestions: parentSurveyQuestions?.length || 0
      });// 1. Міграція основних питань (SurveyQuestion[])
